...
# The raw links the user provided
raw_data = [
...
# Interleaved Progressive Sequence
roadmap_sequence = [
    "BASIC MATHEMATICS", "BASIC MATHS & UNIT/DIMENSIONS",
    "QUADRATIC EQUATIONS", "MOTION IN A STRAIGHT LINE",
    "MOLE CONCEPT", "ATOMIC STRUCTURE",
    "TRIGONOMETRIC FUNCTIONS", "TRIGONOMETRIC EQUATION",
    "TRIGONOMETRY", "MOTION IN A PLANE", "LAWS OF MOTION",
    "PERIODIC TABLE", "CHEMICAL BONDING",
    "WORK, POWER & ENERGY", "Sequence & Series",
    "REDOX REACTIONS", "IUPAC NOMENCLATURE", "GOC",
    "ISOMERISM", "HYDROCARBONS", "GRAVITATION",
    "BINOMIAL THEOREM", "PERMUTATION & COMBINATIONS",
    "STRAIGHT LINES", "CIRCLE", "HYPERBOLA",
    "RELATIONS & FUNCTIONS", "CONTINUITY & METHOD OF DIFFERENTIABILITY",
    "LIMITS", "ROTATIONAL MOTION", "MECHANICAL PROPERTIES OF SOLIDS",
    "OSCILLATIONS", "KTG & THERMODYNAMICS",
    "IONIC EQUILIBRIUM", "THERMODYNAMICS", "THREE-DIMENSIONAL GEOMETRY"
]

lookup = {g['chapterDisplay'].upper(): g for g in processed_goals}

def find_goal(name):
    un = name.upper()
    if un in lookup: return lookup[un]
    for key in lookup:
        if un in key or key in un: return lookup[key]
    return None
...
for name in roadmap_sequence:
    ...
    if goal:
        ...
        key_to_del = next((k for k, v in lookup.items() if v == goal), None)
        if key_to_del: del lookup[key_to_del]

for g in lookup.values(): ordered_list.append(g)

# Assign Days (Strictly 6 hours per day)
current_day = 1
current_day_hours = 0
max_hours_per_day = 6.0

for i, g in enumerate(ordered_list):
    # Buffer slightly to 6.5 to avoid splitting a single long video if it's close
    if (current_day_hours + g['durationHours'] > 6.5) and current_day_hours > 2:
        current_day += 1
        current_day_hours = 0
...
print(f"Roadmap generated: {current_day} days.")
